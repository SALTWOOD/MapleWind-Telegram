import { BotContext, ChatType } from '../../types/index.js';
...
export async function handleSubscribeCommand(ctx: BotContext): Promise<void> {
  const userId = ctx.from?.id;
  const chatId = ctx.chat?.id;
...
export async function handleUnsubscribeCommand(ctx: BotContext): Promise<void> {
  const userId = ctx.from?.id;
  const chatId = ctx.chat?.id;
  const chatType = ctx.chat?.type as ChatType;
...
export async function handleListCommand(ctx: BotContext): Promise<void> {
  const chatId = ctx.chat?.id;
